...
def main():
    lane_id = 1
    ego_vehicle = Vehicle(
        id=0, state=[30, 0 + (lane_id + 0.5) * LANE_WIDTH, 8], lane_id=1
    )
    lane_id = 1
    other_vehicle = Vehicle(
        id=1, state=[45, 0 + (lane_id + 0.5) * LANE_WIDTH, 7], lane_id=1
    )
    ...
    other_vehicle3 = Vehicle(
        id=3, state=[25, 0 + (lane_id + 0.5) * LANE_WIDTH, 6], lane_id=2
    )
    other_vehicle4 = Vehicle(
        id=4, state=[40, 0 + (lane_id + 0.5) * LANE_WIDTH, 6], lane_id=2
    )
    ...
    current_node = mcts.Node(VehicleState([ego_state], actions=[], flow=flow_copy))
    print("root_node:", current_node)

    for t in range(int(prediction_time / DT)):
        print("-------------t=%d----------------" % t)
        old_node = current_node
        current_node = mcts.uct_search(300 / (t / 2 + 1), current_node)
