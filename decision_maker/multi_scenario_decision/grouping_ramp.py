...
def judge_interaction(flow):
    ...
    for i, veh_i in enumerate(flow):
        # 利用横坐标判断所在车道，注意区分匝道和主路
        veh_i_lane_id = get_lane_id(veh_i)
        # 记录车辆是否在merge_zone中
        if (veh_i_lane_id == 0 or veh_i_lane_id == -1) and (RAMP_LENGTH - 20 <= veh_i.current_state.s <= RAMP_LENGTH):
...
def plot_flow(ax, flow, pause_t, target_decision=None):
    ...
    for vehicle in flow:
        ...
        if target_decision:
            if target_decision[vehicle.id][0] in {"cruise", "merge_in"}:
