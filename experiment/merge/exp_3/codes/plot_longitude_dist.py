'''
Author: Wayne@PJLab evenlove2010@gmail.com
Date: 2022-10-26 16:27:26
LastEditors: Wayne@PJLab evenlove2010@gmail.com
LastEditTime: 2022-10-27 20:50:01
FilePath: /merge/exp_3/codes/plot_longitude_dist.py
...
plt.axvline(x=2.5, color=colors['ASBESTOS'], linestyle='--',linewidth=2)
plt.axvline(x=8.3, color=colors['ASBESTOS'], linestyle='--',linewidth=2)
for id in x.keys():
    if id in {0,2,3}:
        if id ==0:
            show_id =4
            show_color = colors['BELIZE HOLE']
            plt.plot(t[:100], x[id][:100]+1,color = show_color, label='Vehicle '+str(show_id),linewidth=3)
        if id == 2:
            show_id =5
