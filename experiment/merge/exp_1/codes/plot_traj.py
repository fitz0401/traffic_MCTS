...
pos_time = -12
xp = trajectories[4][start_time - 30 : end_time - 40]['x'].values - 10
yp = trajectories[4][start_time - 30 : end_time - 40]['y'].values - 4
yawp = trajectories[4][start_time - 30 : end_time - 40]['yaw'].values
x = np.linspace(np.min(xp), np.max(xp), 100)
y = np.interp(x, xp, yp)
yaw = np.interp(x, xp, yawp)
color = gradient_color[-1]
