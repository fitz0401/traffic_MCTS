...
print(trajectories)
plt.axvline(x=3, color=colors['ASBESTOS'], linestyle='--',linewidth=2)
plt.axvline(x=7, color=colors['ASBESTOS'], linestyle='--',linewidth=2)
#get x for each trajectory
# x= { k: v['x'].values for k, v in trajectories.items() }
x= { k: v['vel(m/s)'].values for k, v in trajectories.items() }
print(x)
#plot x for each trajectory
t= np.arange(0, len (x[4]), 1)
t= t*0.1
print(t)
for id in x.keys():
    if id in {0,2,3}:
        if id ==3:
            ...
            plt.plot(t[:100], x[id][:100]-1.0,color = show_color, label='Vehicle '+str(show_id),linewidth=3)
        if id == 2:
            show_id =5
